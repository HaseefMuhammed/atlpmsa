document.getElementById('rollNo').addEventListener('input', function () {
  ...
  studentNameField.value = names[rollNumber] || 'Name not found !!';
});

console.log(`Student database plugin found!!!`);

document.getElementById('rollNo2').addEventListener('input', function () {
  const rollNumber = parseInt(this.value);
  const names = {
    1: 'ARJUN NAND A - 8F',
    2: 'ANURAG P - 8F',
  ...
  const studentNameField = document.getElementById('studentName2');
  studentNameField.value = names[rollNumber] || 'Name not found !!';
});
